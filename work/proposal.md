...
| **Custom route controller** | `pkg/controllers/customroute/custom_route_conditions.go` | Checks OAuth route availability by hitting the route hostname's `/healthz` endpoint | Same situation as endpoint accessibility — `routeAvailability()` uses `http.ProxyFromEnvironment` and needs to use the resolved component proxy instead |
